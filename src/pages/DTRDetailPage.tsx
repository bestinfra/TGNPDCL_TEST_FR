...
const DTRDetailPage = () => {
  ...
  useEffect(() => {
    if (locationHierarchy.length > 0) {
      ...
      if (feederLocation) {
        console.log(
          "Location hierarchy changed, updating address to:",
          feederLocation.name
        );
        setDtr((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    const fetchDtrData = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          console.log(
            "First feeder location data:",
            data.data?.feeders?.[0]?.location
          );
        ...
        if (data.success) {
          ...
          if (data.data?.locationHierarchy) {
            console.log(
              ...
              data.data.locationHierarchy
            );
            console.log(
              "First feeder location:",
              data.data?.feeders?.[0]?.location
            );
    ...
    const fetchAlertsData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${BACKEND_URL}/dtrs/${numericDtrId}/alerts`
        );
                ...
                columns: [
                  {
                    ...
                    props: {
                      ...
                      //     name: 'LastComm', props: { value: lastCosmm }
                      // },
                      rightComponent: {
                        name: "LastComm",
                        props: {
                          // 🔴 LAST SYNC MODE
                          lastSync: true, // Enable special 3-line format

                          // 🔴 CUSTOM TITLES
                          isUpdating: false, 
                          internalTitle: "My Internal", // Row 1 title
                          internalValue: "16:30:02", // Row 1 value
                     

                          // 🔴 CUSTOM VALUES
          
                          dateTitle: "23aug 2025", // Row 2 title
                          dateValue: "14:20:15", // Row 2 value



                          delayedTitle: "My Delayed", // Row 3 title
                          delayedValue: "02:15", // Row 3 value

                          // 🔴 CUSTOM ICONS
                          internalIcon: "icons/check_icon.svg", // Row 1 icon
                          dateIcon: "icons/wifi.svg", // Row 2 icon
                          delayedIcon: "icons/.svg", // Row 3 icon
                        },
                      },
                    },
                  },
