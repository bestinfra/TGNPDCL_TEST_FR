    ...
    useEffect(() => {
        const fetchDropdownData = async () => {
            try {
                ...
                // Fetch roles
                const rolesResponse = await fetch(`${BACKEND_URL}/users/roles`);
                if (rolesResponse.ok) {
                ...
                // Fetch locations
                const locationsResponse = await fetch(`${BACKEND_URL}/users/locations`);
    ...
    const handleFormSubmit = async (formData: Record<string, any>) => {
        ...
        try {
            ...
            if (result.success) {                
                ...
                const successMessage = `User "${userData.firstName} ${userData.lastName}" created successfully!\n\nEmail: ${userData.email}\nPhone: ${userData.phone}\nRole: ${selectedRole?.name || 'Unknown'}\nLocation: ${selectedLocation?.name || 'Unknown'}\nStatus: Active`;
