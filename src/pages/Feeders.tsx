 ...
 // useEffect(() => {}, [consumptionTimeRange, kvaTimeRange]);
  ...
  const handleExportData = () => {
    import("xlsx").then((XLSX) => {
      ...
      const feederInfoExportData = [
        {
          ...
          Capacity: feederInfoData?.dtr?.capacity
            ? `${feederInfoData.dtr.capacity} kVA`
            : "N/A",
          ...
          "Feeder Serial Number":
            feederInfoData?.specificFeeder?.serialNumber || "N/A",
          "Feeder Meter Number":
            feederInfoData?.specificFeeder?.meterNumber || "N/A",
          ...
          Location:
            feederInfoData?.specificFeeder?.location?.name ||
            feederInfoData?.specificFeeder?.city ||
            "N/A",
        },
      ];
      ...
      const consumptionExportData = consumptionAnalyticsData.xAxisData.map(
        (date: string, index: number) => ({
          "S.No": index + 1,
          Date: date || "N/A",
          "Daily Consumption (kWh)":
            consumptionAnalyticsData.seriesData[0]?.data[index] || 0,
          "Monthly Consumption (kWh)":
            consumptionAnalyticsData.monthly?.seriesData[0]?.data[index] || 0,
        })
      );

      // 4. KVA Metrics Data
      const kvaMetricsExportData = kvaMetricsData.xAxisData.map(
        (date: string, index: number) => ({
          "S.No": index + 1,
          Date: date || "N/A",
      ...
      const instantaneousStatsSheet = XLSX.utils.json_to_sheet(
        instantaneousStatsExportData
      );
      ...
      // Generate Excel file
      const feederName =
        effectiveFeederData?.feederName || currentFeederId || "feeder";
